-- Job types (a.k.a. service categories) — admin-managed lookup that drives
...
create index if not exists job_types_active_sort_idx
  on public.job_types(active, sort_order) where active = true;
...
drop policy if exists "job_types: public read active" on public.job_types;
create policy "job_types: public read active" on public.job_types
...
drop trigger if exists job_types_updated_at on public.job_types;
...
insert into public.job_types (slug, label, icon, sort_order) values
